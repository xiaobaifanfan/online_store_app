...
<!-- 		<view class="yt-list">
		...
		<view class="footer">
			...
			<text  class="submit" v-if="pay_status==='paying'">待支付</text>
			<text  class="submit" v-if="pay_status==='TRADE_FINISHED'">交易结束</text>
			<text  class="submit" v-if="pay_status==='WAIT_BUYER_PAY'">交易创建</text>
			<text  class="submit" v-if="pay_status==='TRADE_CLOSED'">超时关闭</text>
...
<script>
	export default {
		...
		onLoad(option){
			...
			this.$request('orders/'+option.id+'/','GET',{},this,function(res){
				...
				res.goods.forEach(item=>{
					that.allPrice += item.goods_num*item.goods.shop_price
				})
